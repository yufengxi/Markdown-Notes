...
> OrderedDict: 让字典记住输入顺序让其变得有序 `d.OrderedDict()`

#### 4. 集合

集合是一种以大括号为定界符的**无序可变序列**，元素间不许重复

方法如下：

>add，update: 增添新元素，并可以忽略重复元素 `s.add(3)`,`s.update({3,4})`  
>pop: 随机删除一个元素  
>remove: 删除一个特定元素，如果指定元素不存在则抛出异常 `s.remove(5)`  
>discard: 删除一个特定元素，如果指定元素不存在则忽略操作 `s.discard(5)`  
>clear: 删除集合全部元素
...
:--------:  | :---------:
.           | 换行符以外的单个字符
\*          | 匹配位于其之前的字符或子模式0次或多次出现
\+          | 匹配位于其之前的字符或子模式至少一次出现
\-          | 在[ ]中表示范围
\|          | 匹配其前或后的字符
'^'         | 匹配行首以其后为开头的字符串
$           | 匹配行尾以其前为结尾的字符串
?           | 匹配其之前的0或1个字符。若其紧随任何限定符其匹配尽可能短的字符串
\           | 转义字符
\num        | num表示正整数，为子模式编号
\f          | 换页符
\n          | 换行符
...
for number in range(1, 100)
    print(number)
#range为内置函数，range(10)代表0-9 10个自然数。，range(x,y)代表x开始n个自然数，range(x,y,z)中z代表步长
```
...
此外，关键字**global**可在函数内创建全局变量，而对于函数本身会创建一个同名的新变量
...
super函数： 调用此函数时，此函数可代替超类并可不提供参数，如：
...
内置函数 `open()`: 打开文件的方法，通常以`f = open('word.txt','a')`的方法打开文件，'a'代表不同的模式
...
> close(): 关闭并保存文件
> write(s): 写入字符串并加到文件后面
> writelines(s): 将字符串写入文本文件，不添加换行符
> read([size]): 读取size个字符，默认读取全部
> readline(): 读取并返回一行字符
> readlines(): 读取每行文本并返回列表
> tell(): 返回文件指针当前位置
> seek(offset, whence): 将指针移动到距whence位置offset的位置

上下文管理语句 with()，可起到try/except的效果。避免发生异常并且文件总能正确关闭:
